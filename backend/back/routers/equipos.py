...
def obtener_equipo(serial: str, db: Session = Depends(get_db)):
...
def entregar_equipo(serial: str, db: Session = Depends(get_db)):
...
def actualizar_equipo(serial: str, equipo_data: schemas.EquipoCreate, db: Session = Depends(get_db)):
